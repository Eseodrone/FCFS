...
//Portage...
#define GPIO_SET_OUTPUT(port, pin)	HAL_GPIO_Init(port, (GPIO_InitTypeDef*)&(const GPIO_InitTypeDef){pin, GPIO_MODE_OUTPUT_PP, GPIO_NOPULL, GPIO_SPEED_FAST, 0})
#define GPIO_SET_INPUT(port, pin)	HAL_GPIO_Init(port, (GPIO_InitTypeDef*)&(const GPIO_InitTypeDef){pin, GPIO_MODE_INPUT, GPIO_NOPULL, GPIO_SPEED_FAST, 0})
...
/*
 * @func	Initialise le LCD.
 * @pre 	ATTENTION, pour fonctionner correctement, la fonction init a besoin qu'un appel périodique soit fait chaque milliseconde à la fonction LCD2X16_process_ms.
 */
